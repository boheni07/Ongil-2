...
/**
 * handover_notes(인수인계) 검증 스키마 — S-21 인수인계 작성 / S-20 목록.
...
/** 인수인계 중요도 — HandoverPriority enum(§schema.prisma)과 1:1. 기본 normal. */
...
/** S-21 인수인계 작성 폼 입력. content 필수, priority 기본 normal, toUserId 대상 지원사. */
export const handoverNoteSchema = z.object({
  toUserId: z.string().uuid("대상 지원사 정보가 올바르지 않습니다."),
  content: z
    .string()
    .min(1, "인수인계 내용을 입력해주세요.")
    .max(3000, "인수인계 내용은 3000자 이내여야 합니다."),
