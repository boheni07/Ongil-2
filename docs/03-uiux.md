...
### 6-8. 소셜 로그인 버튼 (F-AUTH-02, 설계 2026-07-15)

로그인(A-02)·랜딩(A-01) 하단, 이메일 폼 아래 "또는" 구분선 다음에 `SocialAuthButtons` 컴포넌트로 세로 스택 배치한다. 각 버튼은 브랜드 가이드라인 색상·로고를 준수해야 한다(실제 로고 에셋·구현은 후속 라운드).

| 컴포넌트 | 사양 |
|---------|------|
| `SocialAuthButtons` | 이메일 폼 아래 배치. "또는 간편 시작" 텍스트 구분선 + 카카오·네이버 버튼 세로 스택 |
| `KakaoLoginButton` | 배경 `#FEE500`, 텍스트/로고 `#000000` (불투명도 85%). 라벨 "카카오로 시작하기". 카카오 심볼(말풍선) 좌측. 배경색·심볼 변형 금지(브랜드 가이드) |
| `NaverLoginButton` | 배경 `#03C75A`, 텍스트/로고 `#FFFFFF`. 라벨 "네이버로 시작하기". 네이버 `N` 심볼 좌측. 지정 녹색 외 사용 금지(브랜드 가이드) |

- 두 버튼 모두 이메일 버튼과 동일한 높이(터치 타겟 ≥ 56×56px, §5 접근성 기준)·`--br-lg` 라운딩을 따른다.
- 브랜드 요건: 카카오·네이버 모두 **지정 배경색·로고를 임의 변형 금지**한다. 색상 대비가 WCAG AA에 미달하는 조합(카카오 노랑 배경 위 흰 텍스트 등)은 사용하지 않는다.
- 로그인·가입 진입점이 하나의 버튼으로 통합된다("시작하기") — 신규/기존 판별은 콜백(A-11)이 처리하므로 UI에서 로그인/가입을 나누지 않는다.
