...
| `handover_note` | string(≤2000자) | 선택 | 인계인수 메모 |
...
전문가(교사·사회복지사·치료사 등)가 작성 권한을 갖고 기록을 완성하는 구조는 그대로 유지하되, 보호자·당사자가 공식 문서의 존재와 내용을 **인지했음을 남기는 절차**를 추가한다. 반려·차단 기능은 없다 — 이견이 있으면 별도로 정정을 요청한다(§ 관련 커뮤니케이션은 인계인수·알림 재사용).

| 항목 | 결정 |
|---|---|
| 용어 | **확인**(사용 안 함: 승인) — 인계인수(`handover_notes.acknowledged_at`)와 동일한 "읽었음/인지함" 수준 |
...
| F-S-02 | 인계인수 생성·확인 (`acknowledged_at` 기록) | P1 |
...
IEP·치료계획서·회기일지·ISP·전환계획·타임라인·접근 로그·인계인수
