...
export async function getTeacherStudents(): Promise<TeacherStudent[]> {
  ...
  // 보호자는 도메인 제한 없이 모든 기록을 직접 작성할 수 있다(guardians 관계, RecordManager
  // 안내문과 동일 원칙) — 2026-07-19, "구조화 기록도 각 분야 전용 입력폼으로 작성하고 싶다"는
  // 피드백에 따라 담당(permissions) 목록에 자신이 보호자인 당사자를 합쳐 노출한다.
  const personIds = [
    ...new Set([
