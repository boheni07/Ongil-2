...
/**
 * T-19 개별화전환계획(ITP) 목록·학생 선택 — BipListPage.tsx와 동일 구조.
 * 특수교사가 EDU write/edit 권한을 가진 학생 카드. 각 카드는 최근 제출 ITP(다음 검토일·확인
 * 상태) 요약을 보여주고 새 작성으로 유도한다. 목록 자체는 age-block하지 않는다(TRA-001과 동일
 * 관행 — 카드를 눌러 위자드에 진입한 뒤 청소년 전환기가 아니면 안내로 막는다).
 */
export default async function ItpListPage() {
  const students = await getItpClients();
  const total = students.length;
  const missing = students.filter((s) => !s.latestItp).length;

  return (
    <div className="flex flex-1 flex-col">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-headline-2 font-extrabold text-foreground">개별화전환계획(ITP)</h1>
          <p className="mt-1 text-body text-muted-foreground">
            담당 학생 {total}명 · 청소년 전환기 학생의 진로·직업 탐색 계획을 작성·점검하세요.
          </p>
        </div>
        <Button
          render={<Link href="/records/itp/new" />}
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <Stat n={String(missing)} label="ITP 미작성" />
        <Stat n={String(total - missing)} label="ITP 작성됨" />
      </div>
      ...
      {total === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {students.map((s) => (
            <Link
              key={s.personId}
              href={`/records/itp/new?personId=${s.personId}`}
              ...
              {s.latestItp ? (
                <div className="flex flex-col gap-1.5 rounded-(--br-md) bg-muted/50 p-3">
                  <span className="text-caption text-muted-foreground">
                    다음 검토일 {s.latestItp.nextReviewDate ?? "-"}
                  </span>
                  {s.latestItp.requiresConfirmation && (
                    <ConfirmBadge confirmedAt={s.latestItp.confirmedAt} />
                  )}
                </div>
              ) : (
