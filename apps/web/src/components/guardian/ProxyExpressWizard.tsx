...
import { createSelfExpressionForPerson } from "@/app/(app)/persons/[id]/records/actions";
import { IconOption } from "@/components/person/IconOption";
import { SaveOverlay } from "@/components/person/SaveOverlay";
import { WizardProgress } from "@/components/form/WizardProgress";

/**
 * G-22 보호자 대리 자기표현 — docs/07 §5 갭⑥(SELF-001).
 * 당사자가 스스로 기록하기 어려운 경우 보호자가 대신 자기표현(기분·밥·활동·몸 상태)을 남긴다.
 * P-02 자기표현(ExpressWizard) UI를 그대로 재사용하되, 제출 핸들러만
 * createSelfExpressionForPerson(personId, …)로 바꾼 보호자용 변형이다.
 * 저장 시 author_id=보호자, person_id=당사자로 기록돼 감사에서 대리 작성임이 구분된다.
 */
...
export function ProxyExpressWizard({
  personId,
  personName,
}: {
  personId: string;
  ...
  const recordsHref = `/persons/${personId}/records`;
  ...
  async function handleSubmit() {
    ...
    const res = await createSelfExpressionForPerson(personId, input);
    if (res.error) {
      setSaving(false);
      setError(res.error);
      return;
  ...
  return (
    <div className="flex flex-1 flex-col">
      <div className="mb-4">
        <h1 className="text-headline-2 font-extrabold text-foreground">대신 자기표현 남기기</h1>
        <p className="mt-1 text-body text-muted-foreground">
          {personName} 님을 대신해 오늘의 기분·활동을 기록합니다. 대리 작성 사실이 함께 기록됩니다.
        </p>
      </div>

      <WizardProgress current={step} total={4} label={STEP_LABELS[step - 1]} className="mb-8" />
      ...
      <div className="mt-auto flex gap-3 pt-8">
        <button
          type="button"
          onClick={() => (step === 1 ? router.push(recordsHref) : setStep((s) => s - 1))}
