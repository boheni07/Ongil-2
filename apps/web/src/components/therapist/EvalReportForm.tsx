...
export function EvalReportForm({
  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      {/* 대상 아동 선택 */}
      <label className="mt-6 flex max-w-md flex-col gap-1.5">
      ...
      {comparison && (
        <section className="mt-6 flex flex-col gap-3">
          <h2 className="text-label font-bold text-accent-stone">평가 변화 비교 (초기·중간·최종)</h2>
          <EvalComparisonTable comparison={comparison} />
        </section>
      )}

      {loadingContext && (
        <p className="mt-6 text-body text-muted-foreground">치료계획서를 확인하는 중…</p>
      )}

      {noPlan && (
        <div className="mt-6 rounded-xl bg-accent-amber/15 p-5 ring-1 ring-accent-amber/40">
      ...
      {context?.therapyPlanId && !loadingContext && (
        <div className="mt-6 flex flex-col gap-4">
          ...
          <fieldset className="flex flex-col gap-3 rounded-xl border border-border bg-white p-4">
            <legend className="px-1 text-label font-bold text-accent-stone">
              영역별 평가 점수 (0~100)
            </legend>
