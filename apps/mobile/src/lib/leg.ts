...
import { computeLifeStage, isSelfConfirmingStage, type LifeStage } from "./iep";

/**
 * W-17/18/19 LEG(법률·권리) 도메인 사회복지사(social_worker) 데이터 접근(모바일).
 * 웹 Server Action(apps/web/src/app/(app)/records/leg/actions.ts)의 로직을 Supabase 직접
 * 호출로 동일하게 재현한다. DB 계약(테이블·컬럼·record_type 값·requires_confirmation 규칙·
 * 만 19세 진입 가드)은 웹과 1:1 대응하며 임의로 바꾸지 않는다.
 *
 * "담당 당사자"는 이 사회복지사가 LEG 도메인에 활성 write/edit 권한을 가진 persons로 좁힌다
 * (전환계획/TRA 스코핑과 동일 결정). 접근 통제 자체는 전부 기존 RLS(§4-2)에 위임하며
 * 목록 쿼리와 assertSocialWorker 역할 게이트는 앱 레이어 UX 방어선을 서버에서 재확인하는 것이다.
 *
 * requires_confirmation: LEG-001=true(공식 서류 → trg_assign_confirmer가 성인기·노년기=본인,
 * 그 외=주보호자로 확인 주체 자동 지정), LEG-002=false(일상 기록, 확인 절차 없음).
 *
 * 연령 가드: docs/07 §4-4/§4-5에 따라 LEG는 성인기·노년기(만 19세 이상)부터 활성화된다.
 * createGuardianshipReport/createAdvocacyConsultation 양쪽에서 isSelfConfirmingStage로 서버
 * 재검증한다(RLS는 도메인 단위만 강제하므로 이 가드가 없으면 영유아기 당사자에게도 후견감독
 * 보고서 작성이 가능 — 웹 2026-07-17 갭 수정과 동형). logAccess는 iep.ts의 것이 export되지
 * 않아 여기서도 재정의한다(transition.ts/bip.ts와 동일 판단).
 */
 ...
 * 역할이 아니면 false — 최종 방어선은 여전히 RLS다.
 */
async function assertSocialWorker(userId: string): Promise<boolean> {
...
/** 담당 당사자 카드(LEG write/edit 권한 보유자). */
...
export interface LegRecordSummary {
  ...
  /** LEG-001에서만 의미 있음 — 최초 재산목록보고 vs 정기 후견사무보고. */
...
/**
 * 담당 당사자 목록 — 이 사회복지사가 LEG 도메인에 활성 write/edit 권한을 가진 persons.
...
export async function getLegClients(): Promise<LegClient[]> {
  ...
  return persons.map((p) => {
    ...
    const due =
      report &&
      typeof (report.content as { next_report_due?: unknown })?.next_report_due === "string"
 ...
 * 공식 서류라 requires_confirmation=true(§4-6) — 제출 시 trg_assign_confirmer가 확인 주체를
 * 자동 지정한다(앱 코드는 confirmer_id를 다루지 않음).
 * 작성 게이트: social_worker 역할 + 만 19세 이상. DB 최종 방어선은 records_insert RLS.
 */
export async function createGuardianshipReport(
  ...
  if (!(await assertSocialWorker(user.id))) {
 ...
 * consultedAt을 record_date로 사용한다. 파싱 실패 시 서버시각.
 * 작성 게이트: social_worker 역할 + 만 19세 이상. DB 최종 방어선은 records_insert RLS.
 */
