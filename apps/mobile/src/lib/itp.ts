import { itpSchema, type ItpInput, type DomainKey } from "@ongil/validation";
import { supabase } from "./supabase";
import { computeLifeStage, isItpActiveStage, type LifeStage } from "./iep";

/**
 * 개별화전환계획(ITP, EDU-005) 특수교사(teacher) 데이터 접근(모바일) —
 * docs/08-record-taxonomy-workshop.md 안건2-2. 웹 Server Action
 * (apps/web/src/app/(app)/records/itp/actions.ts)의 로직을 Supabase 직접 호출로 동일하게
 * 재현한다. DB 계약(테이블·컬럼·record_type 값·requires_confirmation 규칙·만 13~18세 진입 가드)은
 * 웹과 1:1 대응하며 임의로 바꾸지 않는다.
 *
 * "담당 학생"은 이 교사가 EDU 도메인에 활성 write/edit 권한을 가진 persons로 좁힌다(BIP/전환계획
 * 스코핑과 동일 결정). 접근 통제 자체는 전부 기존 RLS(§4-2)에 위임하며 목록 쿼리는 UX용 사전 필터다.
 *
 ...
 * "연령 가드 누락" 실수 재발 방지). logAccess는 iep.ts의 것이 export되지 않아 재정의한다.
 */
 ...
 * 준비하고 있었다"를 참고하도록 person_id로만 느슨하게 연결한다(FK 없음, 안건2-2).
