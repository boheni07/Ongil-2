...
/**
 * 행동중재계획(BIP, EDU-003) 특수교사(teacher) 데이터 접근(모바일) — docs/07 §5 갭③.
 * 웹 Server Action(apps/web/src/app/(app)/records/bip/actions.ts)의 로직을
 * Supabase 직접 호출로 동일하게 재현한다. DB 계약(테이블·컬럼·record_type 값·
 * requires_confirmation 규칙)은 웹과 1:1 대응하며 임의로 바꾸지 않는다.
 *
 * "담당 학생"은 이 교사가 EDU 도메인에 활성 write/edit 권한을 가진 persons로 좁힌다
 * (전환계획/TRA 스코핑과 동일 결정 — getTeacherStudents의 도메인 무관 스코프는 쓰지 않는다).
 ...
 * logAccess는 iep.ts의 것이 export되지 않아 여기서도 재정의한다(transition.ts와 동일 판단).
 */
 ...
 * (permissions EDU write/edit 분기)가 강제한다. BIP는 연령 가드가 없다(전 생애주기 작성 가능).
 */
