...
/**
 * W-20 복지 타임라인 — 스트림 뷰(시간순) + 도메인 필터. 직전 라운드 EduTimelineScreen은
 * TeacherStack 타입에 묶여 있고 기본 진입 필터·제목이 교육 맥락이라, 복지 맥락(WEL 기본,
 * SocialWorkerStack 타입)으로 최소 복제한다. 데이터 조회(getTimeline)는 domain 무관 범용이라
 * lib/isp 재수출본을 그대로 재사용한다(중복 구현하지 않음).
 */
export function WelTimelineScreen({ route }: Props) {
  ...
  return (
    ...
    >
      <Text style={styles.title}>복지 타임라인{personName ? ` · ${personName}` : ""}</Text>
