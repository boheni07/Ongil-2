...
/**
 * T-20 교육 타임라인 — 스트림 뷰(시간순) + 도메인 필터.
 * 레인 뷰는 모바일 좁은 화면에 맞지 않아 이번 라운드는 생략(스트림 뷰만 구현).
 * 기본 진입은 교육(EDU) 도메인 필터로 시작한다(특수교사 맥락).
 */
export function EduTimelineScreen({ route }: Props) {
  ...
  return (
    ...
    >
      <Text style={styles.title}>교육 타임라인{personName ? ` · ${personName}` : ""}</Text>
