...
--     임의 인증 사용자가 전 당사자의 인수인계·알림을 열람·위조·삭제 가능했다(consents·guardians·
...
-- H1. DAI write 권한자(S1)는 자기 명의로 인수인계 작성 가능
...
SELECT lives_ok(
  ...
  'DAI write 권한 발신자는 인수인계 작성 가능');

-- H2. 권한 없는 사용자(OUT)는 인수인계 작성 불가(RLS WITH CHECK 위반, 42501)
...
SELECT throws_ok(
  ...
  '42501', NULL, '권한(DAI write/보호자) 없는 사용자는 인수인계 작성 불가');
...
SELECT is((SELECT count(*) FROM handover_notes WHERE person_id='a9000000-0000-0000-0000-00000000000c'),
          2::bigint, '수신자는 자기 앞으로 온 인수인계를 열람 가능');

-- H6. 발신자(S1)는 자기 발신 건(1건)만 열람 가능
RESET ROLE; SELECT tests.login('a9000000-0000-0000-0000-000000000001');
SELECT is((SELECT count(*) FROM handover_notes WHERE person_id='a9000000-0000-0000-0000-00000000000c'),
          1::bigint, '발신자는 자기가 보낸 인수인계만 열람 가능');

-- H7. 무관 사용자(OUT)는 인수인계 열람 불가
RESET ROLE; SELECT tests.login('a9000000-0000-0000-0000-000000000003');
SELECT is((SELECT count(*) FROM handover_notes WHERE person_id='a9000000-0000-0000-0000-00000000000c'),
          0::bigint, '발신/수신 당사자가 아니면 인수인계 열람 불가');
...
SELECT throws_ok(
  ...
  '42501', NULL, '인수인계는 DELETE 불가(위조·은폐 방지, GRANT 회수)');
...
INSERT INTO notifications(id, recipient_id, type, title, body)
  VALUES ('a9000000-0000-0000-0000-0000000000e1','a9000000-0000-0000-0000-000000000002','handover','인수인계 도착','확인 요망');
